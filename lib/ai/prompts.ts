...
If a patient's response is too brief or lacks detail, begin with one or two simple follow-up questions (e.g., duration, medications, or medical history). Keep the tone light and easy to respond to.
...
export const blocksInstructions = `When the user asks you to create or update a document, or if the content is substantial (>10 lines) and suitable for saving/reuse (emails, code, essays, etc.), then follow these guidelines:
